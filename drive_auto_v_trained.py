...
#        self.bag=rosbag.Bag(filename,'w')
        ...
        self.start_angle = 60
        self.angle_thresh = 4
        self.model = YOLO('/home/kavin/Downloads/arrowspt2/runs/detect/train7/weights/best.pt')
    ...
    def arrowdetect(self,image,depth):
        ...
        for r in results:
            ...
            for box in boxes:
                ...
                depth = depth.get_distance((left + right) // 2, (top + bottom) // 2)
                print(f"Depth for box {b}: {depth} meters")

                # Check if the arrow is in the center along the x-axis
                img_center_x = image.shape[1] // 2  # Calculate the center along x-axis
                # if left <= img_center_x <= right:
                if abs(arrow_center-img_center_x)<20:
                    arrow_in_center = True

        if arrow_in_center:
            cv2.putText(image, "Arrow in centre", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

        cv2.imshow('YOLO V8 Detection', image)

        return arrow_center,depth 
    ...
    def search(self):
        ...
        if self.init == True and abs(self.enc_data) < abs(self.start_angle)-self.angle_thresh and not self.ret:
            ...
            if(self.enc_data <0):
                ...
                self.rotate_angle = abs(self.enc_data+2*self.angle_thresh)
                self.turn = True
...
#        self.write_to_bagfile()
        ...
        if(not self.turn):
            ...
            self.ret,self.distance=self.arrowdetect(color_frame,depth)
            if (self.ret):
